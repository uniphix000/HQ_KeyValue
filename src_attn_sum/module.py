...
class AttnDecoder(nn.Module):
    ...
    def forward(self, input, h_c, encoder_outputs, k, context_vector, attn_flag=True):
        '''
        ...
        if (attn_flag == True):
            ...
            u_k_t = self.attn_key(torch.cat((context_vector_extend_k, h_t_extend_k, k_extend), 2))  # (b_s, kv, 1)
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, batch_input, batch_output, sentence_lens, keys, encoder, decoder, pad_idx, embed_size, length_limitation=100):
        ...
        for i in range(max_length - 1):
            y_t, h_c = decoder.forward(decoder_input, h_c, encoder_outputs, k, context_vector)  # (batch_size, V)
            if self.training:
