...
class Encoder(nn.Module):
    ...
    def forward(self, batch_input, sentences_lens, keys, pad_idx, batch_size, n, lst):
        ...
        encoder_outputs, _ = pad_packed_sequence(encoder_outputs_packed, batch_first=True)  # ((n-1)*b_s, m_l, b_s)
        # sum
        ...
        if (1):
            ...
            encoder_outputs = [sum([encoder_outputs[i+j] for j in range(0, n-1)], 0)  \
                               for i in range(0, (n-1)*batch_size, (n-1))]
            encoder_outputs = torch.cat(encoder_outputs).view(batch_size, -1, hidden_size)  # (b_s, m_l, h_s)
            encoder_outputs = encoder_outputs.cuda() if use_cuda else encoder_outputs
        return encoder_outputs, (h_last, c_last), keys


class SumDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, dropout, lang, key_size, key_flag='True'):
        ...
        self.key_linear = nn.Linear(2*self.kv, self.kv)
        self.attn_linear = nn.Linear(2*self.hidden_size, self.V)
        self.embedding = nn.Embedding(self.V, self.embed_size)  #

    def forward(self, input, h_c, k, encoder_outputs, context_vector, u_k_t_former, attn_flag=True):
        '''
        ...
        if attn_flag == True:
            ...
            # key预测的拼接
            u_k_t = self.key_linear(torch.cat([u_k_t.squeeze(2), u_k_t_former.squeeze(2)], 1)).unsqueeze(2) #(b_s, kv, 1)
        ...
        h_c = (h_t, c_t)
        return y_t, h_c, u_k_t


class EncoderDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, key_size, dropout, lang,):  # fixme dropout设置成不同的
    ...
    def forward(self, batch_input, batch_output, sentences_lens, keys, encoder, decoder, \
        ...
        encoder_outputs, (h_last, c_last), k = encoder.forward(batch_input, sentences_lens, \
                                                               keys, pad_idx, batch_size, n, lst)  # 这里encoder_outputs是双向的
        u_k_t = Variable(torch.FloatTensor([0.0] * batch_size * (self.kv)).view(batch_size, \
                         -1, 1))
        u_k_t = u_k_t.cuda() if use_cuda else u_k_t
        # decode
        ...
        for i in range(max_length - 1):
            ...
            y_t, h_c, u_k_t = decoder.forward(decoder_input, h_c, k, encoder_outputs, context_vector, u_k_t)  # (batch_size, V)
            if self.training:
