...
def main():
    ...
    encoder_optimizer = optim.Adam(encoder.parameters(), lr=0.01, weight_decay=args.l2)
    decoder_optimizer = optim.Adam(decoder.parameters(), lr=0.01, weight_decay=args.l2)
    encoderdecoder_optimizer = optim.Adam(encoderdecoder.parameters(), lr=args.lr, weight_decay=args.l2)

    encoder_2 = Encoder(args.embed_size, args.hidden_size, args.dropout, lang)
    decoder_2 = SumDecoder(args.embed_size, args.hidden_size, args.dropout, lang, args.key_flag)
    encoderdecoder_2 = EncoderDecoder(args.embed_size, args.hidden_size, args.dropout, lang)
    encoder_2 = encoder_2.cuda() if use_cuda else encoder_2
    decoder_2 = decoder_2.cuda() if use_cuda else decoder_2
    encoderdecoder_2 = encoderdecoder_2.cuda() if use_cuda else encoderdecoder_2
    encoder_optimizer_2 = optim.Adam(encoder_2.parameters(), lr=args.lr, weight_decay=args.l2)
    decoder_optimizer_2 = optim.Adam(decoder_2.parameters(), lr=args.lr, weight_decay=args.l2)
    encoderdecoder_optimizer_2 = optim.Adam(encoderdecoder_2.parameters(), lr=args.lr, weight_decay=args.l2)
    ...
    for i in range(args.max_epoch):
        ...
        for j in range(2, max_utterance_num + 2, 2):
            ...
            # train
            for start_id in range(0, instances_size, args.batch_size):
                ...
                # train
                    ...
                    encoder_2.train()
                    decoder_2.train()
                    encoderdecoder_2.train()
                    encoder_2.zero_grad()
                    decoder_2.zero_grad()
                    encoderdecoder_2.zero_grad()
                    loss = encoderdecoder_2.forward(batch_input, batch_output, sentence_lens, keys_idx, \
                                                  encoder_2, decoder_2, lang.word2idx['pad'], args.embed_size, batch_size, n, lst)
                    loss.backward()
                    clip_grad_norm(encoder_2.parameters(), args.grad_clip)
                    clip_grad_norm(decoder_2.parameters(), args.grad_clip)
                    clip_grad_norm(encoderdecoder_2.parameters(), args.grad_clip)
                    encoder_optimizer_2.step()
                    decoder_optimizer_2.step()
                    encoderdecoder_optimizer_2.step()

        valid_bleu_score, valid_f = evaluate(keys_idx, encoder, decoder, encoderdecoder, encoder_2, decoder_2, encoderdecoder_2, sorted_valid_instances_idx, sorted_valid_answer_dict, lang, \
        ...
        if (valid_f > best_valid_f):
            ...
            test_bleu_score, test_f = evaluate(keys_idx, encoder, decoder, encoderdecoder, encoder_2, decoder_2, encoderdecoder_2, sorted_test_instances_idx, sorted_test_answer_dict, lang, \
                      args.batch_size, args.embed_size, args.hidden_size, args.bleu_path, args.parallel_suffix)
...
def evaluate(keys_idx, encoder, decoder, encoderdecoder, encoder_2, decoder_2, encoderdecoder_2, instances_idx, instances_answer, lang, \
    ...
    for j in range(2, max_utterance_num + 2, 2):
        ...
        for start_id in range(0, instances_size, batch_size):
            ...
            if (j > 2):
                ...
                encoder_2.eval()
                decoder_2.eval()
                encoderdecoder_2.eval()
                batch_predict = encoderdecoder_2.forward(batch_input, batch_output, sentence_lens, keys_idx, \
                                                      encoder_2, decoder_2, lang.word2idx['pad'], embed_size, batch_size, n, lst)
                predict_all.append(batch_predict)
                gold_all.append(batch_gold)
