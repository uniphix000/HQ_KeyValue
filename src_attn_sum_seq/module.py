...
class Encoder(nn.Module):
    def __init__(self, embed_size, hidden_size, dropout, lang):
        ...
        self.seq_lstm = nn.LSTM(self.hidden_size, self.hidden_size, batch_first=True)
    ...
    def forward(self, batch_input, sentences_lens, keys, pad_idx, batch_size, n, lst):
        ...
        if (1):
            ...
            encoder_outputs = torch.cat(encoder_outputs).view(-1, batch_size, n-1, hidden_size)  # (m_l, b_s, n_1, h_s)
            encoder_outputs_box = []
            for i in range(encoder_outputs.size()[0]):  #
                _, (encoder_output, _) = self.seq_lstm(encoder_outputs[i])
                encoder_outputs_box.append(encoder_output)
            encoder_outputs = torch.cat(encoder_outputs_box).view(batch_size, -1, hidden_size)  #
