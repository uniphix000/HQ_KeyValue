...
def main():
    ...
    cmd.add_argument('--manual_data', help='', type = str, default='../data/manual')
    ...
    # 数据预处理: 把标点和词分开,没有标点的统一加上. 并提取kb
    (train_dialogs, train_kb), (valid_dialogs, valid_kb), (test_dialogs, test_kb) = data_preprocess(args.data_path)
    ...
    manual_path = args.manual_data
    lang = generate_dict_manual(lang, manual_path)

    logging.info('最终的dict generated! dict size:{0}'.format(lang.word_size))
    # 存储词典
    ...
    train_instances = generate_instances_manual(manual_path, 'train', train_kb)
    valid_instances = generate_instances_manual(manual_path, 'valid', valid_kb)
    test_instances = generate_instances_manual(manual_path, 'test', test_kb)
    ...
    for i in range(args.max_epoch):
        ...
        for start_id in range(0, train_instances_size, args.batch_size):
            ...
            batch_gold = [train_instances[ids][0] for ids in order[start_id:end_id]]  # 对于train来说没有用,就是为了测试用的，这里是没有变为idx之前的东西
            batch_input, batch_output, _, batch_kb, batch_poi, batch_type, sentence_lens = generate_batch(batch_to_be_generated, batch_gold, batch_size, lang.word2idx['pad'])
            # if(i == 1):
            #     print("a")
            # train
            encoder.train()
            ...
            loss = encoderdecoder.forward(batch_input, batch_output, batch_kb, batch_poi, batch_type, sentence_lens, keys_idx, \
        ...
        logging.info(
            'Round_loss: {0}'.format(Round_loss))
        if (valid_f > best_valid_f):
            ...
            logging.info('New Record! epoch:{0} test F now: {1}  test bleu score now: {2} '.format(\
                i, test_f, test_bleu_score))
...
def evaluate(keys_idx, encoder, decoder, encoderdecoder, instances_idx, instances, lang, \
    ...
    for start_id in range(0, instances_size, batch_size):
        ...
        batch_gold = [instances[ids][0] for ids in order[start_id:end_id]]  # 为了跳过idx的转化
        batch_input, batch_output, batch_gold_output, batch_kb, batch_poi, batch_type, sentence_lens = generate_batch(batch_to_be_generated, batch_gold, batch_size, lang.word2idx['pad'])
        # eval
        encoder.eval()
        decoder.eval()
        encoderdecoder.eval()
        batch_predict = encoderdecoder.forward(batch_input, batch_output, batch_kb, batch_poi, batch_type, sentence_lens, keys_idx, encoder, decoder, lang.word2idx['pad'], embed_size)
    ...
    #print("gold_sentences:{0}".format(gold_sentences))
    length_ = 0
    length_ = sum([len(sentence) for sentence in predict_sentences])
    #print("predict_sentences:{0}".format(predict_sentences))
    #print("length_{0}".format(length_))
    if length_==0:
        predict_sentences[0]+='I'
