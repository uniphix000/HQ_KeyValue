...
class Encoder(nn.Module):
    def __init__(self, embed_size, hidden_size, dropout, lang):
        ...
        self.linear = nn.Linear(self.embed_size, self.hidden_size)
        self.cos = nn.CosineSimilarity(dim=0)

    def forward(self, input, batch_kb, batch_poi, batch_type, sentence_lens, keys, pad_idx):
        '''
        这里要得到kj
        ...
        k = torch.sum(embed_key, 1)  # (kv, embe d_size)

        embed_kb = self.embedding(batch_kb.view(-1, 2)).view(batch_size, 8, 2, -1)
        h_kb = torch.sum(embed_kb, 2)  # (b_s, 8, e_s)

        # poi_embed = torch.sum(self.embedding(batch_poi.view(-1, 4)), 1).view(-1, 8, self.embed_size)  # (b_s, 8, e_s)
        # type_embed = torch.sum(self.embedding(batch_type.view(-1, 4)), 1).view(-1, 8, self.embed_size)  #
        # kb_embed = poi_embed + type_embed
        # kb_hidden = self.linear(kb_embed)
        # print (kb_hidden.size())
        ...
        kb_cos = torch.cat(flatten([[self.cos(h_last[0][i], h_kb[i][j]) for j in range(8)] \
                                       for i in range(batch_size)])).view(batch_size, -1)

        _, kb_select = torch.max(kb_cos, 1)
        kb_hidden = torch.cat([torch.index_select(h_kb[i], 0, kb_select[i]) for i in range(batch_size)])

        kb_idx = torch.cat([torch.index_select(batch_kb[i], 0, kb_select[i]) for i in range(batch_size)])

        # print (kb_hidden)
        # kb_cos = torch.cat(flatten([[self.cos(h_last[0][i], kb_hidden[i][j]) for j in range(8)] \
        #                                for i in range(batch_size)])).view(batch_size, -1)
        # _, kb_select = torch.max(kb_cos, 1)
        # print (batch_kb)
        # print (len(kb_select));print (kb_select);exit(0)


        return encoder_outputs, (h_last, c_last), k, kb_hidden, kb_idx
...
class AttnDecoder(nn.Module):
    def __init__(self, embed_size, hidden_size, dropout, lang, key_flag='True'):
        ...
        self.poi_linear = nn.Linear(self.hidden_size, self.embed_size)
    ...
    def forward(self, input, h_c, encoder_outputs, k, kb_hidden, kb_idx, context_vector, attn_flag=True):
        '''
        ...
        if (attn_flag == True):
            ...
            context_vector_extend_k = torch.cat([context_vector.unsqueeze(1)] * kv, 1)
            kb_hidden_k = torch.cat([kb_hidden.unsqueeze(1)] * kv, 1)
            u_k_t = self.attn_key(torch.cat((k_extend, kb_hidden_k), 2)) # (b_s, kv, 1)
            # u_k_t = Variable(torch.FloatTensor([0.0] * batch_size * self.V).view(batch_size, \
            #              -1, 1))
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, batch_input, batch_output, batch_kb, batch_poi, batch_type, sentence_lens, keys, encoder, decoder, pad_idx, embed_size, length_limitation=100):
        '''
        ...
        encoder_outputs, (h_last, c_last), k, kb_hidden, kb_idx = encoder.forward(batch_input, batch_kb, batch_poi, batch_type, sentence_lens, keys, pad_idx)  # 这里encoder_outputs是双向的
        ...
        for i in range(max_length - 1):
            y_t, h_c = decoder.forward(decoder_input, h_c, encoder_outputs, k, kb_hidden, kb_idx, context_vector)  # (batch_size, V)
            if self.training:
