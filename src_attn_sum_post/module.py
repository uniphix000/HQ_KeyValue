...
class Encoder(nn.Module):
    ...
    def forward(self, batch_input, sentences_lens, keys, pad_idx, batch_size, n, lst):
        ...
        if (1):
            ...
            encoder_outputs = torch.cat(encoder_outputs).view(batch_size * (n-1), -1, hidden_size)  #
            encoder_outputs = encoder_outputs.cuda() if use_cuda else encoder_outputs
        return encoder_outputs, (h_last, c_last), keys

class SumDecoder(nn.Module):
    def __init__(self, batch_size, embed_size, hidden_size, dropout, lang, key_flag='True'):
    ...
    def forward(self, input, h_c, k, encoder_outputs, context_vector, attn_flag=True):
        '''
        ...
        if attn_flag == True:
            batch_size = len(h_t)
            n = (encoder_outputs.size()[0] // batch_size) + 1
            h_t_extend = torch.cat([h_t.unsqueeze(1)] * encoder_outputs.size()[1], 1)  # (b_s, m_l, h_s)
            h_t_unsorted = [h_t_extend] * (n-1)
            h_t_sorted = flatten([[h_t_unsorted[i][j] for i in range(0, n-1)] \
                                           for j in range(0, batch_size)])
            h_t_extend = torch.cat(h_t_sorted).view((n-1)*batch_size, -1, self.hidden_size)
            u_t = self.attn(torch.cat((encoder_outputs, h_t_extend), 2))  # ((n-1)*b_s, m_l, 1)
            a_t = self.ssoftmax(u_t.squeeze(2)).unsqueeze(2)
            h_t_ = torch.sum((a_t * encoder_outputs), 1)  # ((n-1)*b_s, h_s)
            h_t_ = [sum([h_t_[i+j] for j in range(0, n-1)], 0)  for i in range(0, (n-1)*batch_size, (n-1))]
            h_t_ = torch.cat(h_t_).view(batch_size, self.hidden_size)
            kv = len(k)
...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, batch_input, batch_output, sentences_lens, keys, encoder, decoder, \
        ...
        for i in range(max_length - 1):
            y_t, h_c = decoder.forward(decoder_input, h_c, k, encoder_outputs, context_vector)  # (batch_size, V)
            if self.training:
